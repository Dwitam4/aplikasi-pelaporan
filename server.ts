...
async function readDocument<T>(key: string, fallback: T): Promise<T> {
  if (hostedDb) {
    const value = (await hostedDb.query('SELECT value FROM app_documents WHERE key=$1', [key])).rows[0]?.value;
    if (value === undefined || value === null) return fallback;
    return (typeof value === 'string' ? readJson(value, fallback) : value) as T;
  }
...
app.get("/api/health", async (_req, res) => {
  try {
    res.json({ status: "ok", timestamp: now(), reportsCount: await reportsCount(), activeSSECount: sseClients.length, database: usingSupabase ? "Supabase PostgreSQL" : "SQLite local fallback" });
  } catch (error: any) {
    console.error("Database health check failed:", error);
    res.status(503).json({ status: "degraded", timestamp: now(), database: usingSupabase ? "Supabase PostgreSQL" : "SQLite local fallback", error: "Database unavailable" });
  }
});
